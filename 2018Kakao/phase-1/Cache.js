...
**  이 프로그램의 테스팅 업무를 담당하고 있는 어피치는 서비스를 오픈하기 전 각 로직에 대한 성능 측정을 수행하였는데, 제이지가 작성한 부분 중 데이터 베이스에서
    게시물을 가져오는 부분의 실행시간이 너무 오래 걸린다는 것을 알게 되었다.
**  어피치는 제이지에게 해당 로직을 개선하라고 닦닭하기 시작하였고, 제이지는 DB 캐시를 적용하여 성능 개선을 시도하고 있지만 캐시 크기를 얼마로 해야 효율적인지
    몰라 난감한 상황이다.
...
**  입력된 도시이름 배열을 순서대로 처리할 대, "총 실행시간"을 출력한다.
*/
...
function solution(cacheSize, cities){
  ...
  for(var i = 0; i< cities.length; i++){
    if(stack.length<cacheSize){
      ...
      if(stack.indexOf(cities[i].toLowerCase()) == -1){
        stack.shift();
        stack.push(cities[i].toLowerCase());
        answer += CACHE_MISS;
      }else{
        stack.splice(stack.indexOf(cities[i].toLowerCase()),1);
        stack.push(cities[i].toLowerCase());
        answer += CACHE_HIT;
      }
    }
...
console.log(solution(test1.cacheSize, test1.cities));
console.log(solution(test2.cacheSize, test2.cities));
console.log(solution(test3.cacheSize, test3.cities));
console.log(solution(test4.cacheSize, test4.cities));
console.log(solution(test5.cacheSize, test5.cities));
console.log(solution(test6.cacheSize, test6.cities));
